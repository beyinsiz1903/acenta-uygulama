...
async def approve_hotel_booking(booking_id: str, request: Request, user=Depends(get_current_user)):
    ...
    # Reuse booking_id as stable idempotency key
    idempotency_key = booking_id

    # Build PMS payload from booking (minimal fields)
    pms_payload = {
        "hotel_id": booking.get("hotel_id"),
        "agency_id": booking.get("agency_id"),
        "stay": booking.get("stay"),
        "occupancy": booking.get("occupancy"),
        "guest": booking.get("guest"),
        "special_requests": booking.get("special_requests"),
        "rate_snapshot": booking.get("rate_snapshot"),
    ...
    await write_booking_event(
        ...
        payload={"source": "hotel_panel_pms"},
    )
