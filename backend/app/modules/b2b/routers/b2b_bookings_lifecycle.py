"""B2B booking post-confirm lifecycle routers (T009 / Task #3).

Extracted from `b2b_bookings.py`.

Owns:
- POST /api/b2b/bookings/{id}/refund-requests   (create_refund_request)
- POST /api/b2b/bookings/{id}/cancel            (cancel_b2b_booking)
- POST /api/b2b/bookings/{id}/amend/quote       (amend_booking_quote)
- POST /api/b2b/bookings/{id}/amend/confirm     (amend_booking_confirm)
- GET  /api/b2b/bookings/{id}/events            (get_booking_events)

External contract (paths, method signatures, response shapes, ledger
postings, audit log entries, idempotency semantics) preserved exactly.
"""
