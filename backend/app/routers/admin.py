...
async def create_hotel(payload: HotelCreateIn, user=Depends(get_current_user)):
    ...
    # Default package: basic (can be changed later by admin)
    if not doc.get("package"):
        doc["package"] = "basic"
...
@router.patch("/hotels/{hotel_id}/package", dependencies=[Depends(require_roles(["super_admin"]))])
async def patch_hotel_package(hotel_id: str, payload: dict, user=Depends(get_current_user)):
    """Update hotel package (basic|pro|channel).

    This is a minimal feature flag mechanism used to control Channel Hub access.
    """
    db = await get_db()
    existing = await db.hotels.find_one({"organization_id": user["organization_id"], "_id": hotel_id})
    if not existing:
        raise HTTPException(status_code=404, detail="Otel bulunamadı")

    package = payload.get("package")
    if not package:
        raise HTTPException(status_code=400, detail="package field required")

    now = now_utc()
    await db.hotels.update_one(
        {"_id": hotel_id},
        {"$set": {"package": package, "updated_at": now, "updated_by": user.get("email")}},
    )
