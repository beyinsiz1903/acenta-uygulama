#!/usr/bin/env python3
"""
Stripe Webhook Finalize Guard Test
Testing the new payments_finalize_guard behavior at backend level
"""
...
BASE_URL = "https://agency-analysis-1.preview.emergentagent.com"
...
async def test_duplicate_event_dedupe():
    ...
    try:
        # Import and call the guard function directly
        import sys
...
async def test_webhook_router_integration():
    ...
    # Simulate webhook payload (without signature verification)
    webhook_payload = {
    ...
    try:
        # Note: We can't easily test the webhook endpoint directly due to signature verification
        # Instead, we'll test the handler function directly which is what the webhook calls
        
        import sys
        sys.path.append('/app/backend')
        from app.services.stripe_handlers import handle_stripe_webhook
        
        # Create mock raw body and signature (this will fail signature verification)
        # For testing purposes, we'll call the guard function directly
        from app.services.payments_finalize_guard import apply_stripe_event_with_guard
