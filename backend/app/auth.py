...
async def get_current_user_optional(credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)) -> Optional[dict[str, Any]]:
    """Optional auth guard.
...
def require_roles(required: list[str]):
    async def _dep(user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
        ...
        # super_admin her zaman geçsin
        if is_super_admin(user):
            return user

        if roles.intersection(set(required)):
            return user

        # Rol uygun değilse, yapılandırılmış bir hata döndür
        raise HTTPException(
            status_code=403,
            detail={
...
def require_hotel_capability(capability: str):
    """Guard that enforces hotel-level capabilities based on hotel.package.

    For now, this is a simple mapping:
    - basic:    {"booking_approval"}
    - pro:      {"booking_approval", "reports"}
    - channel:  {"booking_approval", "reports", "channel_hub"}
    """
    PACKAGE_CAPS = {
        "basic": {"booking_approval"},
        "pro": {"booking_approval", "reports"},
    ...
    async def _guard(user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
        ...
        if not hotel_id or not org_id:
            raise HTTPException(status_code=403, detail={"code": "NO_HOTEL_CONTEXT", "message": "Otel yetkisi bulunamadı"})

        db = await get_db()
        hotel = await db.hotels.find_one({"organization_id": org_id, "_id": str(hotel_id)})
        if not hotel:
            raise HTTPException(status_code=404, detail="Otel bulunamadı")

        package = hotel.get("package") or "basic"
        caps = PACKAGE_CAPS.get(package, PACKAGE_CAPS["basic"])
        if capability not in caps:
            raise HTTPException(
                status_code=403,
                detail={
                    "code": "FEATURE_NOT_AVAILABLE",
                    "message": "Bu özellik paketinizde aktif değil.",
                },
            )
