...
# Backend URL: https://b2b-dashboard-3.preview.emergentagent.com

# Step 1: Login as admin and get JWT token
echo "🔐 Step 1: Admin Login"
LOGIN_RESPONSE=$(curl -s -X POST "https://b2b-dashboard-3.preview.emergentagent.com/api/auth/login" \
...
curl -s -X GET "https://b2b-dashboard-3.preview.emergentagent.com/api/ops/finance/settlements" \
...
CREATE_RESPONSE=$(curl -s -X POST "https://b2b-dashboard-3.preview.emergentagent.com/api/ops/finance/settlements" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
curl -s -X GET "https://b2b-dashboard-3.preview.emergentagent.com/api/ops/finance/settlements?supplier_id=$SUPPLIER_ID&currency=EUR" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" | jq '.'
...
curl -s -X POST "https://b2b-dashboard-3.preview.emergentagent.com/api/ops/finance/settlements" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
