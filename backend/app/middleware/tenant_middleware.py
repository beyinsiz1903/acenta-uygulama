...
class TenantResolutionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):  # type: ignore[override]
        ...
        if (
            ...
            or path.startswith("/api/agency/")
            or path.startswith("/api/ai-assistant/")
            or path.startswith("/api/system/")
            or path.startswith("/api/products")
            or path.startswith("/api/customers")
            or path.startswith("/api/leads")
            or path.startswith("/api/quotes")
            or path.startswith("/api/reservations")
            ...
            or path.startswith("/api/match-")
            or path.startswith("/api/inbox")
            or path.startswith("/api/advanced-reports")
            or path.startswith("/api/hotel")
            or path.startswith("/api/commission")
            or path.startswith("/api/marketplace")
