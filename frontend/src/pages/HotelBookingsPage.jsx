...
export default function HotelBookingsPage() {
  ...
  const [sourceFilter, setSourceFilter] = useState("all"); // all | public | other
  ...
  async function submitAction() {
    ...
    try {
      if (actionType === "confirm") {
        // PMS-late approve: only for pending bookings
        await api.post(`/hotel/bookings/${actionBooking.id}/approve`, {});
      } else if (actionType === "reject") {
        // Hotel-side rejection with optional reason
        await api.post(`/hotel/bookings/${actionBooking.id}/reject`, {
      ...
      await loadBookings();
      toast({
        title: actionType === "confirm" ? "Talep onaylandı" : "Talep reddedildi",
        duration: 2500,
      });
    } catch (err) {
      // Map common PMS / workflow errors to user-friendly messages
      ...
      const baseMessage =
        ...
        (typeof detail === "string" ? detail : apiErrorMessage(err));

      let msg = baseMessage;
      const normalizedCode = String(code || "").toUpperCase();

      if (status === 503 || normalizedCode === "PMS_UNAVAILABLE") {
        msg = "PMS şu an erişilemiyor. Lütfen birkaç dakika sonra tekrar deneyin.";
      } else if (normalizedCode === "NO_INVENTORY") {
        msg = "Seçilen tarihler için artık müsait oda kalmadı.";
      } else if (normalizedCode === "PRICE_CHANGED") {
        msg = "Fiyat PMS tarafında değişti. Lütfen acentadan yeniden teklif alın.";
      } else if (
        status === 409 &&
        (normalizedCode === "INVALID_STATE_TRANSITION" || typeof detail === "string")
      ) {
        msg = "Rezervasyon durumu değişmiş görünüyor. Lütfen listeyi yenileyip tekrar deneyin.";
      }
  ...
  const filtered = useMemo(() => {
    ...
    let bySource = byGroup;
    if (sourceFilter === "public") {
      bySource = bySource.filter((b) => b.source === "public_booking");
  ...
  }, [enrichedBookings, agencyQuery, filterGroup, sourceFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between mt-2">
        ...
        <div className="inline-flex rounded-full bg-muted px-1 py-1 text-xs">
          <button
            className={`px-3 py-1 rounded-full ${sourceFilter === "all" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
            onClick={() => setSourceFilter("all")}
          >
            Tüm Kaynaklar
          </button>
          <button
            className={`px-3 py-1 rounded-full ${sourceFilter === "public" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
            onClick={() => setSourceFilter("public")}
          >
            Public Talepler
          </button>
          <button
            className={`px-3 py-1 rounded-full ${sourceFilter === "other" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
              ...
              filtered.map((booking) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className="font-medium">{guest.full_name || customer.name || "-"}</div>
                        {isPublic ? <Badge variant="outline">Public Talep</Badge> : null}
                      </div>
                      <div className="text-xs text-muted-foreground mt-0.5">
                        ...
                        {!customer.name && !customer.phone && !customer.email && guest.email && (
