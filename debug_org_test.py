...
def test_api_with_debug():
    ...
    try:
        ...
        response = requests.post("https://resflow-polish.preview.emergentagent.com/api/auth/login", json=login_data, timeout=10)
        ...
        response = requests.get("https://resflow-polish.preview.emergentagent.com/api/crm/customers", headers=headers, timeout=10)
        ...
        response = requests.get("https://resflow-polish.preview.emergentagent.com/api/crm/customers?search=seed", headers=headers, timeout=10)
        print(f"Status: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"Response: {data}")
