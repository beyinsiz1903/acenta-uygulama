...
async def get_pilot_metrics_dashboard() -> dict[str, Any]:
    ...
    return {
        ...
        "financial_flow": {
            ...
            "reconciliation_mismatch_rate": round(100 - _rate([m for m in metrics if m.get("step") == "reconciliation_check"]), 2),
        },
        "pilot_usage": {
...
async def _record_incident(db, agency_name: str, step: str, detail: dict[str, Any]):
    severity = "critical" if step in ("booking_test", "accounting_sync_test") else "high"
    doc = {
        "agency_name": agency_name,
        "step": step,
        "severity": severity,
