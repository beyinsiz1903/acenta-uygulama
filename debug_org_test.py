...
def test_api_with_debug():
    ...
    try:
        ...
        response = requests.get("https://test-data-populator.preview.emergentagent.com/api/crm/customers", headers=headers, timeout=10)
        ...
        response = requests.get("https://test-data-populator.preview.emergentagent.com/api/crm/customers?search=seed", headers=headers, timeout=10)
        print(f"Status: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"Response: {data}")
