...
@pytest.mark.anyio
async def test_booking_amend_quote_and_confirm_increase_delta(async_client, agency_token):
    """P1.5: Amend flow (quote + confirm) with positive delta.

    Senaryo:
    - EUR bir booking yarat (manual insert) ve sell_eur=100.0 kabul et
    - Yeni tarih icin inventory fiyatini daha yuksek ayarla
    ...
    # 2) EUR hotel product sec (catalog tarzı)
    ...
    # 3) Iki tarih icin inventory olustur: ikinci tarih daha pahali
    today = now_utc().date()
    check_in_1 = today + timedelta(days=10)
    check_out_1 = check_in_1 + timedelta(days=1)
    check_in_2 = today + timedelta(days=20)
    check_out_2 = check_in_2 + timedelta(days=1)

    date1 = check_in_1.isoformat()
    date2 = check_in_2.isoformat()

    # Base fiyatlar: 100 -> 150 (delta > 0 bekliyoruz)
    ...
    # 4) Baslangic booking'i manuel olustur (CONFIRMED, EUR, sell_eur=100)
    now = now_utc()
    booking_doc = {
        "organization_id": org_id,
        "agency_id": agency_id,
    ...
    # 9) Ledger: BOOKING_AMENDED delta posting
