...
async def generate_match_risk_executive_pdf(
  db,
  *,
  organization_id: str,
  ...
  if not docs:
    ...
    snapshot_date = generated_at.date().isoformat() if hasattr(generated_at, "date") else None
  ...
  await track_report_generated(
    ...
    tenant_id=tenant_id,
    report_type="match_risk_executive_summary",
    output_format="pdf",
    source="reports.match_risk",
    source_event_id=f"{correlation_id}:match-risk-executive:{snapshot_date or today_str}",
    metadata={
      "report_key": "match_risk_daily",
      "filename": filename,
