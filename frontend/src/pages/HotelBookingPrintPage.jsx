...
export default function HotelBookingPrintPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Otel tarafı için tekil booking endpoint'ini kullan
        const resp = await api.get(`/hotel/bookings/${id}`);
        if (!mounted) return;
        setBooking(resp.data || null);
      } catch (e) {
        if (!mounted) return;
        toast({
          title: "Yüklenemedi",
