#!/usr/bin/env python3
"""
Phase-1 Multi-tenant Backend Test for Acenta Master
...
class MultiTenantTester:
    def __init__(self, base_url="https://hotel-pricing-engine.preview.emergentagent.com"):
        ...
        if success:
            ...
            if len(links) >= 3:
                ...
                for link in links:
                    ...
                    # Store the first agency1->hotel2 link for visibility test
                    if not self.agency1_hotel2_link_id and link.get('active'):
    ...
    def test_patch_agency_hotel_link(self):
        """Test PATCH /api/admin/agency-hotel-links/{id} active=false"""
        if not self.agency1_hotel2_link_id:
            self.log("⚠️  No link ID available for patch test")
            return False
            
        success, response = self.run_test(
            f"Admin - Deactivate Link {self.agency1_hotel2_link_id}",
            "PATCH",
        ...
        if success:
            if response.get('active') == False:
                self.log(f"✅ Link deactivated successfully")
                return True
            else:
    ...
    def test_verify_link_deactivation(self):
        """Verify the link was deactivated in the list"""
        success, response = self.run_test(
            "Admin - Verify Link Deactivation",
            "GET",
            "api/admin/agency-hotel-links",
            200,
            token=self.super_admin_token
        ...
        if success:
            ...
            if deactivated_link and deactivated_link.get('active') == False:
                self.log(f"✅ Link deactivation verified in list")
                return True
            else:
                self.log(f"❌ Link deactivation not reflected in list")
                return False
        return False
    ...
    def test_visibility_after_deactivation(self):
        ...
        self.log("\n=== 5. VISIBILITY TEST AFTER DEACTIVATION ===")
        success, response = self.run_test(
            "Agency1 - Verify Hotel Visibility After Link Deactivation",
            "GET",
            "api/agency/hotels",
            200,
            token=self.agency1_token
        ...
        if success:
            ...
            # After deactivating one link, agency1 should see fewer hotels
            if len(hotels) < 2:
                self.log(f"✅ Visibility rule working - Agency1 now sees {len(hotels)} hotel(s): {hotel_names}")
                return True
            else:
                self.log(f"❌ Visibility rule failed - Agency1 still sees {len(hotels)} hotels: {hotel_names}")
    ...
    def print_summary(self):
        """Print test summary"""
        self.log("\n" + "="*60)
        self.log("PHASE-1 MULTI-TENANT TEST SUMMARY")
        self.log("="*60)
    ...
    def run_all_tests(self):
        ...
        self.log("🚀 Starting Phase-1 Multi-Tenant Tests")
        ...
        # Test 5: Visibility after deactivation
