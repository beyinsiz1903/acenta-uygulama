...
async def portal_login(payload: Dict[str, Any], db=Depends(get_db)):
    ...
    booking = await db.bookings.find_one(
        {"guest_email": {"$regex": f"^{email}$", "$options": "i"}, "confirmation_code": booking_code},
        {"_id": 0},
    )
    if not booking:
        booking = await db.reservations.find_one(
            {"guest_email": {"$regex": f"^{email}$", "$options": "i"}, "code": booking_code},
            {"_id": 0},
        )
    if not booking:
...
async def my_bookings(token: str = Query(...), db=Depends(get_db)):
    ...
    filt: Dict[str, Any] = {"guest_email": {"$regex": f"^{email}$", "$options": "i"}}
    if org_id:
    ...
    res_cursor = db.reservations.find(
        {"guest_email": {"$regex": f"^{email}$", "$options": "i"}, "organization_id": org_id} if org_id else {"guest_email": {"$regex": f"^{email}$", "$options": "i"}},
...
async def my_booking_detail(booking_code: str, token: str = Query(...), db=Depends(get_db)):
    ...
    booking = await db.bookings.find_one(
        {"confirmation_code": booking_code, "guest_email": {"$regex": f"^{session['email']}$", "$options": "i"}},
        {"_id": 0},
    )
    if not booking:
        booking = await db.reservations.find_one(
            {"code": booking_code, "guest_email": {"$regex": f"^{session['email']}$", "$options": "i"}},
            {"_id": 0},
        )
    if not booking:
...
async def my_documents(token: str = Query(...), db=Depends(get_db)):
    ...
    cursor = db.invoices.find(
        {"organization_id": org_id, "customer_email": {"$regex": f"^{email}$", "$options": "i"}},
    ...
    voucher_cursor = db.vouchers.find(
        {"organization_id": org_id, "guest_email": {"$regex": f"^{email}$", "$options": "i"}},
