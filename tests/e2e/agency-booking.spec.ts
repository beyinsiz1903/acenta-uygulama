...
test.describe("AdminMetricsPage FAZ-12.1+13.2 metrics smoke", () => {
  ...
  test("T4 - Hotels tab shows search and table", async ({ page }) => {
    ...
    await page.getByTestId("metrics-tab-hotels").click();

    await expect(page.getByTestId("metrics-hotels-search")).toBeVisible();
    await expect(page.getByTestId("metrics-hotels-table")).toBeVisible();
  });
