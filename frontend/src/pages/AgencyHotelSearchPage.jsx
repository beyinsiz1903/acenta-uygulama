...
export default function AgencyHotelSearchPage() {
  ...
  // Derived search data (hooks must be before any early return)
  ...
  const filteredRatePlansByRoomKey = (() => {
  ...
  const selectedRoom = (() => {
    ...
    return rooms.find((r) => r.room_type_id === selected.room_type_id) || null;
  })();

  const selectedRatePlan = (() => {
    if (!selectedRoom || !selected) return null;
    return (selectedRoom.rate_plans || []).find((rp) => rp.rate_plan_id === selected.rate_plan_id) || null;
  })();
