export const FEATURE_CATALOG = [
  { key: "dashboard", label: "Dashboard", description: "Ana kontrol paneli" },
  { key: "partners", label: "Partnerler", description: "Partner ağı yönetimi" },
  { key: "b2b", label: "B2B Ağ", description: "B2B borsa ve acente ağı" },
  { key: "accounting", label: "Muhasebe", description: "Muhasebe ve fatura yönetimi" },
  { key: "webpos", label: "Web POS", description: "Online satış noktası" },
  { key: "reports", label: "Raporlar", description: "Raporlama ve analitik" },
  { key: "inventory", label: "Envanter", description: "Müsaitlik ve envanter yönetimi" },
  { key: "crm", label: "CRM", description: "Müşteri ilişkileri yönetimi" },
  { key: "ops", label: "Operasyon", description: "Operasyon yönetimi" },
];
