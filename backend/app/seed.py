...
async def ensure_seed_data() -> None:
    ...
    await db.tours.create_index([("organization_id", 1), ("status", 1), ("created_at", -1)])
    await db.tour_booking_requests.create_index([
        ("agency_id", 1), ("status", 1), ("created_at", -1)
    ])
    await db.tour_booking_requests.create_index([
        ("tour_id", 1), ("created_at", -1)
    ])
    ...
    if not org:
        org_doc = {
            ...
            "payment_offline": {
                ...
                "account_holder": "Syroce Demo Hesap",
                "bank_name": "Demo Bankası",
                "note": "Açıklamaya referans kodunu yazınız."
            },
            "allow_agency_mark_paid": False,
        }
        res = await db.organizations.insert_one(org_doc)
        org_id = str(res.inserted_id)
    else:
        org_id = str(org["_id"])
        # Backfill payment_offline & allow_agency_mark_paid if missing
        update_fields = {}
        if "payment_offline" not in org:
            update_fields["payment_offline"] = {
                "enabled": True,
                "iban": "TR00 0000 0000 0000 0000 0000 00",
                "account_holder": "Syroce Demo Hesap",
                "bank_name": "Demo Bankası",
        ...
        if "allow_agency_mark_paid" not in org:
            update_fields["allow_agency_mark_paid"] = False
        if update_fields:
            update_fields["updated_at"] = now_utc()
            await db.organizations.update_one({"_id": org["_id"]}, {"$set": update_fields})
    ...
    # Submit idempotency (per organization)
    await db.bookings.create_index(
        [("organization_id", 1), ("idempotency_key", 1)],
        unique=True,
        sparse=True,
        name="idx_bookings_org_idempotency_key",
    )
    ...
    # Phase-2.2: Channel Hub collections
    await db.channel_connectors.create_index(
        [("organization_id", 1), ("hotel_id", 1), ("provider", 1)],
        unique=True,
        name="idx_channel_connectors_org_hotel_provider",
    )
    await db.channel_connectors.create_index(
        [("organization_id", 1), ("hotel_id", 1), ("status", 1)],
        name="idx_channel_connectors_status",
    )

    await db.channel_mappings.create_index(
        [("organization_id", 1), ("hotel_id", 1), ("connector_id", 1)],
        unique=True,
        name="idx_channel_mappings_org_hotel_connector",
    )

    await db.channel_sync_runs.create_index(
        [
            ("organization_id", 1),
            ("hotel_id", 1),
            ("connector_id", 1),
        ...
        unique=False,
        name="idx_channel_sync_runs_key",
    )

    await db.pms_daily_rates.create_index(
        [
            ("organization_id", 1),
            ("hotel_id", 1),
            ("pms_rate_plan_id", 1),
        ...
        unique=False,
        name="idx_pms_daily_rates_key",
    )

    await db.pms_daily_availability.create_index(
            [
                ("organization_id", 1),
                ("hotel_id", 1),
                ("pms_room_type_id", 1),
            ...
            unique=False,
            name="idx_pms_daily_availability_key",
        )
    ...
    await db.channel_sync_runs.create_index(
        ...
        unique=False,
        name="idx_channel_sync_runs_idem_key",
    )
    ...
    # Phase-2.0: agency hotel catalog (public sales config)
    await db.agency_hotel_catalog.create_index(
